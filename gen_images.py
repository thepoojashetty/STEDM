...
def main(cfg : DictConfig):
    ...
    predict_dir = test_folder_path + "/syn_data_for_report/" + f"{cfg.run_name}"
    Path(predict_dir).mkdir(parents=True, exist_ok=True)

    for k in range(4):
        # load test condition image
        test_img = np.array(Image.open(test_folder_path + f"/test_{k+1}.png").convert('L'))
        ...
        with torch.no_grad():
            ...
            if cfg.style_sampling.name == "nearby" or cfg.style_sampling.name == "augmented":
                ...
                for i in range(cfg.style_sampling.num_patches):
                    style_0.append((torch.from_numpy(np.array(Image.open(test_style_path + f"/0_img_{i}.png"))[:,:,:3]).to(torch.float32).to(module.device).unsqueeze(0).unsqueeze(0) / 127.5)-1)
                    style_1.append((torch.from_numpy(np.array(Image.open(test_style_path + f"/1_img_{i}.png"))[:,:,:3]).to(torch.float32).to(module.device).unsqueeze(0).unsqueeze(0) / 127.5)-1)
                    style_2.append((torch.from_numpy(np.array(Image.open(test_style_path + f"/2_img_{i}.png"))[:,:,:3]).to(torch.float32).to(module.device).unsqueeze(0).unsqueeze(0) / 127.5)-1)
                    # style_3.append((torch.from_numpy(np.array(Image.open(test_style_path + f"/3_img_{i}.png"))[:,:,:3]).to(torch.float32).to(module.device).unsqueeze(0).unsqueeze(0) / 127.5)-1)
            ...
            img_ddim_w1, _ = module._model.sample_log(c_1, batch_size=1, ddim=True, ddim_steps=cfg.ddim_steps, eta=cfg.eta, log_every_t=1000, unconditional_conditioning=c_uncond, unconditional_guidance_scale=cfg.cfg_scale)
            img_ddim_w2, _ = module._model.sample_log(c_2, batch_size=1, ddim=True, ddim_steps=cfg.ddim_steps, eta=cfg.eta, log_every_t=1000, unconditional_conditioning=c_uncond, unconditional_guidance_scale=cfg.cfg_scale)
            # img_ddim_w3, _ = module._model.sample_log(c_3, batch_size=1, ddim=True, ddim_steps=cfg.ddim_steps, eta=cfg.eta, log_every_t=1000, unconditional_conditioning=c_uncond, unconditional_guidance_scale=cfg.cfg_scale)
            ...
            # save img
            Image.fromarray(img_ddim_w0).save(predict_dir + f"/img_{k+1}0.png")
            Image.fromarray(img_ddim_w1).save(predict_dir + f"/img_{k+1}1.png")
            Image.fromarray(img_ddim_w2).save(predict_dir + f"/img_{k+1}2.png")
            ...
            Image.fromarray(seg_0).save(predict_dir + f"/seg_{k+1}0.png")
